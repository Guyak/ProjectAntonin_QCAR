 ...
 * C source code generated on : Wed Oct 26 11:06:28 2022
 *
...
void Stop_StudentVersion_output(void)
{
  ...
  if (rtmIsMajorTimeStep(Stop_StudentVersion_M)) {
    ...
    if (0.0 > Stop_StudentVersion_P.commandsaturation_UpperSat) {
      rtb_Add1 = Stop_StudentVersion_P.commandsaturation_UpperSat;
    } else if (0.0 < Stop_StudentVersion_P.commandsaturation_LowerSat) {
      rtb_Add1 = Stop_StudentVersion_P.commandsaturation_LowerSat;
    } else {
      rtb_Add1 = 0.0;
    }
     ...
     */
    Stop_StudentVersion_B.desired = rtb_Add1 *
  ...
  /* Sum: '<S10>/Sum' */
  rtb_Sum_i = Stop_StudentVersion_B.desired - Stop_StudentVersion_B.measured;
  if (rtmIsMajorTimeStep(Stop_StudentVersion_M)) {
   ...
   *  Sum: '<S10>/Add1'
   */
  rtb_Add1 = ((Stop_StudentVersion_P.Kpms_Gain * rtb_Sum_i +
               Stop_StudentVersion_X.Integrator1_CSTATE_o) +
  ...
  if (rtb_Add1 > Stop_StudentVersion_P.commandsaturation_UpperSat_e) {
    /* Saturate: '<S3>/command saturation' */
    Stop_StudentVersion_B.commandsaturation =
      Stop_StudentVersion_P.commandsaturation_UpperSat_e;
  } else if (rtb_Add1 < Stop_StudentVersion_P.commandsaturation_LowerSat_h) {
    /* Saturate: '<S3>/command saturation' */
    Stop_StudentVersion_B.commandsaturation =
      Stop_StudentVersion_P.commandsaturation_LowerSat_h;
  } else {
    /* Saturate: '<S3>/command saturation' */
    Stop_StudentVersion_B.commandsaturation = rtb_Add1;
  }
  ...
  if (rtmIsMajorTimeStep(Stop_StudentVersion_M)) {
     ...
     *  Gain: '<S3>/Gain'
     */
    rtb_Add1 = Stop_StudentVersion_P.Gain_Gain_j *
  ...
  /* Gain: '<S10>/Ki (% // m)  ' */
  Stop_StudentVersion_B.Kim = Stop_StudentVersion_P.Kim_Gain * rtb_Sum_i;
  if (rtmIsMajorTimeStep(Stop_StudentVersion_M)) {
    ...
    if (Stop_StudentVersion_B.RGB_o4) {
      ...
      /* Gain: '<S16>/Gain' */
      rtb_Sum_i = Stop_StudentVersion_P.Gain_Gain *
      ...
      if (Stop_StudentVersion_B.Constant ==
        ...
        rtb_Add1 = Stop_StudentVersion_P.Constant_Value_m;
        rtb_Sum_i = Stop_StudentVersion_P.Constant1_Value_j;
      } else {
        rtb_Add1 = rtb_Sum_i - Stop_StudentVersion_P.HueThreshold_Value;
        rtb_Sum_i += Stop_StudentVersion_P.HueThreshold_Value;
      }
      ...
      /* S-Function Block: Stop_StudentVersion/findStopSignLocation/colorThresholdingHSV/HSV Color Thresholding (image_compare_block) */
      {
        ...
        if ((comparisons[0] & COMPARE_WRAP) == 0) {
          ...
          if (rtb_Sum_i < 0) {
            maximums[0] = 0;
          } else if (rtb_Sum_i > 255U) {
            maximums[0] = 255U;
          } else {
            maximums[0] = (t_uint8) rtb_Sum_i;
          }
        } else {
          minimums[0] = (t_uint8) (rtb_Add1 - 256U * floor(rtb_Add1 / 256U));
          maximums[0] = (t_uint8) (rtb_Sum_i - 256U * floor(rtb_Sum_i / 256U));
        }
    ...
    if (rtb_Depth_o4 && Stop_StudentVersion_B.Compare_c) {
      ...
      rtb_Sum_i = Stop_StudentVersion_P.AdjustforVFOV_Gain *
        Stop_StudentVersion_B.Subtract[0];

      /* Gain: '<S7>/Adjust for H-FOV' */
      rtb_Add1 = Stop_StudentVersion_P.AdjustforHFOV_Gain *
       ...
       *  Sum: '<S7>/Add1'
       */
      rtb_Sum_i = floor(Stop_StudentVersion_P.Constant_Value + rtb_Sum_i);
      if (rtIsNaN(rtb_Sum_i) || rtIsInf(rtb_Sum_i)) {
        rtb_Sum_i = 0.0;
      } else {
        rtb_Sum_i = fmod(rtb_Sum_i, 65536.0);
      }

      rtb_Add1 = floor(rtb_Add1 + Stop_StudentVersion_P.Constant1_Value);
      ...
      Stop_StudentVersion_B.Selector = Stop_StudentVersion_B.Depth_o1
        [(((rtb_Add1 < 0.0 ? (int32_T)(int16_T)-(int16_T)(uint16_T)-rtb_Add1 :
            (int32_T)(int16_T)(uint16_T)rtb_Add1) - 1) * 720 + (rtb_Sum_i < 0.0 ?
           (int32_T)(int16_T)-(int16_T)(uint16_T)-rtb_Sum_i : (int32_T)(int16_T)
           (uint16_T)rtb_Sum_i)) - 1];
      if (rtmIsMajorTimeStep(Stop_StudentVersion_M)) {
...
RT_MODEL_Stop_StudentVersion_T *Stop_StudentVersion(void)
{
  ...
  /* External mode info */
  Stop_StudentVersion_M->Sizes.checksums[0] = (1939481674U);
  Stop_StudentVersion_M->Sizes.checksums[1] = (4217873779U);
  Stop_StudentVersion_M->Sizes.checksums[2] = (2054657824U);
  Stop_StudentVersion_M->Sizes.checksums[3] = (2554826546U);
