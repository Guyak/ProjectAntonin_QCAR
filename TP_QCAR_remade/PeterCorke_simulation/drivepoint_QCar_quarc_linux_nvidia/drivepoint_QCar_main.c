...
typedef RT_MODEL_drivepoint_QCar_T RT_MODEL;

/*====================*
 * External functions *
 *====================*/
EXTERN RT_MODEL * drivepoint_QCar(void);
  ...
  _do_assertion(const char * expression, const char * file_name, int line_number)
{
  ...
  rtmSetErrorStatus(drivepoint_QCar_M, GBLbuf.message);
}
  ...
  main(int argc, char * argv[])
{
  ...
  if (parseError) {
    (void) fprintf(stderr,
                   "\nUsage: drivepoint_QCar -option1 val1 -option2 val2 -option3 ...\n\n");
    (void) fprintf(stderr,
