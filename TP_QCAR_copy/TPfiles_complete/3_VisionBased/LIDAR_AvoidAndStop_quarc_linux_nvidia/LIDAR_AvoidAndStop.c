...
void LIDAR_AvoidAndStop_output0(void)  /* Sample time: [0.0s, 0.0s] */
{
  ...
  if (rtmIsMajorTimeStep(LIDAR_AvoidAndStop_M)) {
     ...
     */
    for (i = 0; i < 720; i++) {
      LIDAR_AvoidAndStop_B.Y[i] = ((LIDAR_AvoidAndStop_B.correctedDistances[i] <=
        0.2) && (LIDAR_AvoidAndStop_B.correctedDistances[i] >= 0.05));
      calc4_0 = (int8_T)((LIDAR_AvoidAndStop_B.correctedDistances[i] <= 1.0) &&
                         (LIDAR_AvoidAndStop_B.correctedDistances[i] >= 0.1));
      calc1[i] = (int8_T)((LIDAR_AvoidAndStop_B.Constant1[i] <=
                           0.52359877559829882) * calc4_0);
      calc4[i] = (int8_T)((LIDAR_AvoidAndStop_B.Constant1[i] >=
                           5.7595865315812871) * calc4_0);
    }
  ...
  /* Limited  Integrator  */
  if (rtmIsMajorTimeStep(LIDAR_AvoidAndStop_M)) {
    y = (((LIDAR_AvoidAndStop_PrevZCX.Integrator1_Reset_ZCE == 1) != (int32_T)
...
void LIDAR_AvoidAndStop_initialize(void)
{
  ...
  /* S-Function Block: LIDAR_AvoidAndStop/HIL Initialize1 (hil_initialize_block) */
  {
    ...
    result = hil_set_card_specific_options
      (LIDAR_AvoidAndStop_DW.HILInitialize1_Card, " ", 2);
    if (result < 0) {
      msg_get_error_messageA(NULL, result, _rt_error_message, sizeof
        (_rt_error_message));
      rtmSetErrorStatus(LIDAR_AvoidAndStop_M, _rt_error_message);
...
RT_MODEL_LIDAR_AvoidAndStop_T *LIDAR_AvoidAndStop(void)
{
  ...
  /* External mode info */
  LIDAR_AvoidAndStop_M->Sizes.checksums[0] = (888133993U);
  LIDAR_AvoidAndStop_M->Sizes.checksums[1] = (3178198735U);
  LIDAR_AvoidAndStop_M->Sizes.checksums[2] = (3868220984U);
  LIDAR_AvoidAndStop_M->Sizes.checksums[3] = (2072971573U);
  ...
  LIDAR_AvoidAndStop_M->Sizes.numBlocks = (64);/* Number of blocks */
  LIDAR_AvoidAndStop_M->Sizes.numBlockIO = (25);/* Number of block outputs */
