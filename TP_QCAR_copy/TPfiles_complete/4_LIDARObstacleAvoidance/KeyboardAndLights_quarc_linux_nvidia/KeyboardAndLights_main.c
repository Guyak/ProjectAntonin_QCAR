...
typedef RT_MODEL_KeyboardAndLights_T RT_MODEL;

/*====================*
 * External functions *
 *====================*/
EXTERN RT_MODEL * KeyboardAndLights(void);
  ...
  _do_assertion(const char * expression, const char * file_name, int line_number)
{
  ...
  rtmSetErrorStatus(KeyboardAndLights_M, GBLbuf.message);
}
  ...
  main(int argc, char * argv[])
{
  ...
  if (parseError) {
    (void) fprintf(stderr,
                   "\nUsage: KeyboardAndLights -option1 val1 -option2 val2 -option3 ...\n\n");
    (void) fprintf(stderr,
