% Setup file for Path_following.slx for plotting the trajectories
%modified by Mayank JHA
...
Y_ic = x0(2); % (m) random ICs on Y position
%yaw_ic = 2*pi*(rand(nAgents,1)-0.5); % (rad) random ICs for yaw, or heading
yaw_ic = x0(3); % (rad) random ICs for yaw, or heading
...
plotAxisLimits = [-3 4 -3 5]; % [xmin xmax ymin ymax]
