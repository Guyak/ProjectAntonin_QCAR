...
typedef RT_MODEL_LIDAR_Stop_T RT_MODEL;

/*====================*
 * External functions *
 *====================*/
EXTERN RT_MODEL * LIDAR_Stop(void);
  ...
  _do_assertion(const char * expression, const char * file_name, int line_number)
{
  ...
  rtmSetErrorStatus(LIDAR_Stop_M, GBLbuf.message);
}
  ...
  main(int argc, char * argv[])
{
  ...
  if (parseError) {
    (void) fprintf(stderr,
                   "\nUsage: LIDAR_Stop -option1 val1 -option2 val2 -option3 ...\n\n");
    (void) fprintf(stderr,
