...
typedef RT_MODEL_Manual_control_to_overtakel_T RT_MODEL;

/*====================*
 * External functions *
 *====================*/
EXTERN RT_MODEL * Manual_control_to_overtakel(void);
  ...
  _do_assertion(const char * expression, const char * file_name, int line_number)
{
  ...
  rtmSetErrorStatus(Manual_control_to_overtakel_M, GBLbuf.message);
}
  ...
  main(int argc, char * argv[])
{
  ...
  if (parseError) {
    (void) fprintf(stderr,
                   "\nUsage: Manual_control_to_overtakel -option1 val1 -option2 val2 -option3 ...\n\n");
    (void) fprintf(stderr,
