...
void Stop_StudentVersion_output(void)
{
  ...
  if (rtmIsMajorTimeStep(Stop_StudentVersion_M)) {
     ...
     *  Constant: '<Root>/m//s'
     */
    if ((0.1 < Stop_StudentVersion_B.Selector) < 0.4) {
      rtb_SteeringBias = 0.0;
    } else {
...
RT_MODEL_Stop_StudentVersion_T *Stop_StudentVersion(void)
{
  ...
  /* External mode info */
  Stop_StudentVersion_M->Sizes.checksums[0] = (933344294U);
  Stop_StudentVersion_M->Sizes.checksums[1] = (554150948U);
  Stop_StudentVersion_M->Sizes.checksums[2] = (543327173U);
  Stop_StudentVersion_M->Sizes.checksums[3] = (964625757U);
