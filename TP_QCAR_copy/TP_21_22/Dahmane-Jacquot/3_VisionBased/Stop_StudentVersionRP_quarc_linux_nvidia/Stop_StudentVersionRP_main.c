...
typedef RT_MODEL_Stop_StudentVersionR_T RT_MODEL;

/*====================*
 * External functions *
 *====================*/
EXTERN RT_MODEL * Stop_StudentVersionRP(void);
  ...
  _do_assertion(const char * expression, const char * file_name, int line_number)
{
  ...
  rtmSetErrorStatus(Stop_StudentVersionRP_M, GBLbuf.message);
}
  ...
  main(int argc, char * argv[])
{
  ...
  if (parseError) {
    (void) fprintf(stderr,
                   "\nUsage: Stop_StudentVersionRP -option1 val1 -option2 val2 -option3 ...\n\n");
    (void) fprintf(stderr,
            ...
            if (result == 0) {
              ...
              while (result == 0) {
...

                {
                  ...
                  if (local_result < 0) {
                    msg_get_error_messageA(NULL, local_result, _rt_error_message,
