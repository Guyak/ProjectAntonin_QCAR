...
void Path_following_TP_output0(void)   /* Sample time: [0.0s, 0.0s] */
{
  ...
  if (!searchFlag) {
    memcpy(&Path_following_TP_DW.obj.WaypointsInternal[0],
           &Path_following_TP_P.elipse[0], 140U * sizeof(real_T));
    Path_following_TP_DW.obj.ProjectionLineIndex = 0.0;
  }

  for (trueCount = 0; trueCount < 140; trueCount++) {
    b[trueCount] = !rtIsNaN(Path_following_TP_P.elipse[trueCount]);
  }
  ...
  for (trueCount = 0; trueCount < 2; trueCount++) {
    for (b_k = 0; b_k < f_size_idx_0; b_k++) {
      waypoints_data[b_k + f_size_idx_0 * trueCount] =
        Path_following_TP_P.elipse[(70 * trueCount + f_data[b_k]) - 1];
    }
  }
...
RT_MODEL_Path_following_TP_T *Path_following_TP(void)
{
  ...
  /* External mode info */
  Path_following_TP_M->Sizes.checksums[0] = (741907018U);
  Path_following_TP_M->Sizes.checksums[1] = (1415262448U);
  Path_following_TP_M->Sizes.checksums[2] = (751856272U);
  Path_following_TP_M->Sizes.checksums[3] = (1827064490U);
  ...
  Path_following_TP_M->Sizes.numBlockPrms = (299);/* Sum of parameter "widths" */
