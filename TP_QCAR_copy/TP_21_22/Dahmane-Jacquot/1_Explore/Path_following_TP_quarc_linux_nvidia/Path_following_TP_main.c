...
typedef RT_MODEL_Path_following_TP_T RT_MODEL;

/*====================*
 * External functions *
 *====================*/
EXTERN RT_MODEL * Path_following_TP(void);
  ...
  _do_assertion(const char * expression, const char * file_name, int line_number)
{
  ...
  rtmSetErrorStatus(Path_following_TP_M, GBLbuf.message);
}
  ...
  main(int argc, char * argv[])
{
  ...
  if (parseError) {
    (void) fprintf(stderr,
                   "\nUsage: Path_following_TP -option1 val1 -option2 val2 -option3 ...\n\n");
    (void) fprintf(stderr,
