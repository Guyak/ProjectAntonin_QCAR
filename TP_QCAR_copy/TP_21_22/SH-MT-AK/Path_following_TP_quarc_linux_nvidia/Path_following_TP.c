 ...
 * C source code generated on : Fri Jan 21 10:47:49 2022
 *
...
static void Pa_PurePursuitBase_stepInternal(nav_slalgs_internal_PurePursu_T *obj,
  const real_T currentPose[3], const real_T wayptsIn[6004], real_T *v, real_T *w,
  ...
  real_T lookaheadPoint[2], real_T *targetDir)
{
  ...
  int32_T trueCount;
  for (i = 0; i < 6004; i++) {
    Path_following_TP_B.b[i] = !rtIsNaN(wayptsIn[i]);
  }

  trueCount = 0;
  for (i = 0; i < 3002; i++) {
    if (Path_following_TP_B.b[i] && Path_following_TP_B.b[i + 3002]) {
      trueCount++;
    }
  }
  ...
  for (i = 0; i < 3002; i++) {
    if (Path_following_TP_B.b[i] && Path_following_TP_B.b[i + 3002]) {
      Path_following_TP_B.f_data[trueCount] = (int16_T)(i + 1);
      trueCount++;
    }
  }
  ...
  for (i = 0; i < 2; i++) {
    for (trueCount = 0; trueCount < f_size_idx_0; trueCount++) {
      Path_following_TP_B.waypoints_data[trueCount + f_size_idx_0 * i] =
        wayptsIn[(3002 * i + Path_following_TP_B.f_data[trueCount]) - 1];
    }
  }
...
void Path_following_TP_output0(void)   /* Sample time: [0.0s, 0.0s] */
{
  ...
  if (!p) {
    memcpy(&Path_following_TP_DW.obj.WaypointsInternal[0],
           &Path_following_TP_P.p4[0], 6004U * sizeof(real_T));
  ...
  Pa_PurePursuitBase_stepInternal(&Path_following_TP_DW.obj, tmp,
    Path_following_TP_P.p4, &Path_following_TP_B.PurePursuit_o1, &rtb_L2psidot,
    a__1, &rtb_sinpsi);
...
RT_MODEL_Path_following_TP_T *Path_following_TP(void)
{
  ...
  /* External mode info */
  Path_following_TP_M->Sizes.checksums[0] = (2018510576U);
  Path_following_TP_M->Sizes.checksums[1] = (4227878520U);
  Path_following_TP_M->Sizes.checksums[2] = (1093458716U);
  Path_following_TP_M->Sizes.checksums[3] = (3068668789U);
  ...
  Path_following_TP_M->Sizes.numBlockPrms = (6163);/* Sum of parameter "widths" */
