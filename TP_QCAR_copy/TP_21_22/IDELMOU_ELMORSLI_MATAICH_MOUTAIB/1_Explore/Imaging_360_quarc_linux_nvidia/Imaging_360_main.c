...
typedef RT_MODEL_Imaging_360_T RT_MODEL;

/*====================*
 * External functions *
 *====================*/
EXTERN RT_MODEL * Imaging_360(void);
  ...
  _do_assertion(const char * expression, const char * file_name, int line_number)
{
  ...
  rtmSetErrorStatus(Imaging_360_M, GBLbuf.message);
}
  ...
  main(int argc, char * argv[])
{
  ...
  if (parseError) {
    (void) fprintf(stderr,
                   "\nUsage: Imaging_360 -option1 val1 -option2 val2 -option3 ...\n\n");
    (void) fprintf(stderr,
                  ...
                  if (local_result == 0) {
                    local_result = timeout_subtract
                      (&Imaging_360_DW.ComputationTime_ComputationTime,
                       &end_time, &Imaging_360_DW.ComputationTime_BeginTime);
                  }
