...
typedef RT_MODEL_Manual_Drive_T RT_MODEL;

/*====================*
 * External functions *
 *====================*/
EXTERN RT_MODEL * Manual_Drive(void);
  ...
  _do_assertion(const char * expression, const char * file_name, int line_number)
{
  ...
  rtmSetErrorStatus(Manual_Drive_M, GBLbuf.message);
}
  ...
  main(int argc, char * argv[])
{
  ...
  if (parseError) {
    (void) fprintf(stderr,
                   "\nUsage: Manual_Drive -option1 val1 -option2 val2 -option3 ...\n\n");
    (void) fprintf(stderr,
                  ...
                  if (local_result == 0) {
                    local_result = timeout_subtract
                      (&Manual_Drive_DW.ComputationTime_ComputationTime,
                       &end_time, &Manual_Drive_DW.ComputationTime_BeginTime);
                  }
