...
static void Pa_PurePursuitBase_stepInternal(nav_slalgs_internal_PurePursu_T *obj,
  const real_T currentPose[3], const real_T wayptsIn[1258], real_T *v, real_T *w,
  ...
  for (i = 0; i < 2; i++) {
    for (trueCount = 0; trueCount < f_size_idx_0; trueCount++) {
      Path_following_TP_B.waypoints_data[trueCount + f_size_idx_0 * i] =
        wayptsIn[(629 * i + f_data[trueCount]) - 1];
    }
  }
...
void Path_following_TP_output0(void)   /* Sample time: [0.0s, 0.0s] */
{
  ...
  if (!p) {
    memcpy(&Path_following_TP_DW.obj.WaypointsInternal[0],
           &Path_following_TP_P.p2[0], 1258U * sizeof(real_T));
  ...
  Pa_PurePursuitBase_stepInternal(&Path_following_TP_DW.obj, tmp,
    Path_following_TP_P.p2, &Path_following_TP_B.PurePursuit_o1, &rtb_L2psidot,
    a__1, &rtb_sinpsi);
...
RT_MODEL_Path_following_TP_T *Path_following_TP(void)
{
  ...
  /* External mode info */
  Path_following_TP_M->Sizes.checksums[0] = (3578838489U);
  Path_following_TP_M->Sizes.checksums[1] = (2310218526U);
  Path_following_TP_M->Sizes.checksums[2] = (199625702U);
  Path_following_TP_M->Sizes.checksums[3] = (4009779772U);
  ...
  Path_following_TP_M->Sizes.numBlockPrms = (1417);/* Sum of parameter "widths" */
