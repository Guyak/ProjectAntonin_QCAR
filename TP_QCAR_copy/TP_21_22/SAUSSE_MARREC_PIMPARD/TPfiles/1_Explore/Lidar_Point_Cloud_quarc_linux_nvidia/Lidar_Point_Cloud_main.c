...
typedef RT_MODEL_Lidar_Point_Cloud_T RT_MODEL;

/*====================*
 * External functions *
 *====================*/
EXTERN RT_MODEL * Lidar_Point_Cloud(void);
  ...
  _do_assertion(const char * expression, const char * file_name, int line_number)
{
  ...
  rtmSetErrorStatus(Lidar_Point_Cloud_M, GBLbuf.message);
}
  ...
  main(int argc, char * argv[])
{
  ...
  if (parseError) {
    (void) fprintf(stderr,
                   "\nUsage: Lidar_Point_Cloud -option1 val1 -option2 val2 -option3 ...\n\n");
    (void) fprintf(stderr,
