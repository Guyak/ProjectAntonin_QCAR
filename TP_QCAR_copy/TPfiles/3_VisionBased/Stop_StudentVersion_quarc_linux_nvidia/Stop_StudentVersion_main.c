...
typedef RT_MODEL_Stop_StudentVersion_T RT_MODEL;

/*====================*
 * External functions *
 *====================*/
EXTERN RT_MODEL * Stop_StudentVersion(void);
  ...
  _do_assertion(const char * expression, const char * file_name, int line_number)
{
  ...
  rtmSetErrorStatus(Stop_StudentVersion_M, GBLbuf.message);
}
  ...
  main(int argc, char * argv[])
{
  ...
  if (parseError) {
    (void) fprintf(stderr,
                   "\nUsage: Stop_StudentVersion -option1 val1 -option2 val2 -option3 ...\n\n");
    (void) fprintf(stderr,
