...
typedef RT_MODEL_RGBD_Imaging_T RT_MODEL;

/*====================*
 * External functions *
 *====================*/
EXTERN RT_MODEL * RGBD_Imaging(void);
  ...
  _do_assertion(const char * expression, const char * file_name, int line_number)
{
  ...
  rtmSetErrorStatus(RGBD_Imaging_M, GBLbuf.message);
}
  ...
  main(int argc, char * argv[])
{
  ...
  if (parseError) {
    (void) fprintf(stderr,
                   "\nUsage: RGBD_Imaging -option1 val1 -option2 val2 -option3 ...\n\n");
    (void) fprintf(stderr,
                  ...
                  if (local_result == 0) {
                    local_result = timeout_subtract
                      (&RGBD_Imaging_DW.ComputationTime_ComputationTime,
                       &end_time, &RGBD_Imaging_DW.ComputationTime_BeginTime);
                  }
