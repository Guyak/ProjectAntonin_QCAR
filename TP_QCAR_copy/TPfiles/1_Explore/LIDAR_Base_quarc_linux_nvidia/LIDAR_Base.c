...
void LIDAR_Base_output0(void)          /* Sample time: [0.0s, 0.0s] */
{
  ...
  if (rtmIsMajorTimeStep(LIDAR_Base_M)) {
     ...
     */
    for (i = 0; i < 720; i++) {
      Y1 = ((LIDAR_Base_B.correctedDistances[i] <= 1.0) &&
            (LIDAR_Base_B.correctedDistances[i] >= 0.1));
      calc1[i] = (int8_T)((LIDAR_Base_B.Constant1[i] <= 0.52359877559829882) *
                          Y1);
      calc4[i] = (int8_T)((LIDAR_Base_B.Constant1[i] >= 5.7595865315812871) * Y1);
    }
  ...
  /* Limited  Integrator  */
  if (rtmIsMajorTimeStep(LIDAR_Base_M)) {
    Y1 = (((LIDAR_Base_PrevZCX.Integrator1_Reset_ZCE == 1) != (int32_T)
...
void LIDAR_Base_initialize(void)
{
  ...
  /* S-Function Block: LIDAR_Base/HIL Initialize1 (hil_initialize_block) */
  {
    ...
    is_switching = false;
    result = hil_set_card_specific_options(LIDAR_Base_DW.HILInitialize1_Card,
      " ", 2);
    if (result < 0) {
      msg_get_error_messageA(NULL, result, _rt_error_message, sizeof
        (_rt_error_message));
...
RT_MODEL_LIDAR_Base_T *LIDAR_Base(void)
{
  ...
  /* External mode info */
  LIDAR_Base_M->Sizes.checksums[0] = (377160317U);
  LIDAR_Base_M->Sizes.checksums[1] = (3331593002U);
  LIDAR_Base_M->Sizes.checksums[2] = (790644782U);
  LIDAR_Base_M->Sizes.checksums[3] = (2854591517U);
